...
for i in range(epochs):
    ...
    for idx in range(n_training_steps):
        ...
        # first hidden layer
        y_1 = np.matmul(x, weights_1) + bias_1 # why x is first? (bias - column vector)
        y_1 = sigmoid(y_1) # column vector
        # output
        y_2 = np.matmul(y_1, weights_2) + bias_2 # column vector (just one row)
        y_2 = sigmoid(y_2)
        
        diff = y_2 - y # 1 element matrix (column vector)

        # mean squared error ?
        mse = np.sum(diff) / (2.0 * y_2.shape[0])
        print(idx, mse)
        
        grad_2 = y_2 * (1 - y_2) # sigmoid derivative
        d_2 = diff * grad_2 # 1 element matrix (column vector)
        # np.sum(dZ2,axis=1,keepdims=True)
        dw2 = d_2 * y_1
        weights_2 += learning_rate * dw2.T
...
def network_forward(x):
    y_1 = np.matmul(x, weights_1) + bias_1
    y_1 = sigmoid(y_1)
    y_2 = np.matmul(y_1, weights_2) + bias_2
    y_2 = sigmoid(y_2)
    return y_2
