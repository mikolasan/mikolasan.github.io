...
well, everyone is building artificial general intelligence or AGI. I’m gonna build so why SOYSOI not sorry that I was not so beans.
Which of course, means speckles of intelligence


Everyone is so obsessed with LLMs that they use it to create high level plans and then apply it to some random technique like reinforcement learning or search in a graph or to write some code. But this leads to low accuracy because of the conversion back and forth. Transformers and attention is a good technique and one doesn’t need to use language for their tasks. Teach it from the beginning in a domain that one needs. 
Although the problem in this case arises. Where to find gigabytes of training data?

## Algorithms that write algorithms 
First, we need a driving force - emotions. When inference matches expected output, then the sequence of events is memorized and can be retrieved as an algorithm. In case of a failure, it spans another copy of current algorithm and checks every step for an improvement.
- if the output is known and we are playing a game of finding a difference or transformation process, does that output play a role as input?
- How helpful to work it out by pixels or steps?
- We need high level concepts like lines. Do we learn them from one example? Probably not. But we need invariants: seeing a line in one position should not limit us to find a line in another position or even another scale of pixels (4 pixels as 1).
